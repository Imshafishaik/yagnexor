...
async function seedDatabase() {
  ...
  try {
    ...
    for (const course of courses) {
      await db.query(`
        INSERT INTO courses (id, tenant_id, name, code, description, department_id, duration_months, credits)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ...
    for (const cls of classes) {
      await db.query(`
        INSERT INTO classes (id, tenant_id, name, course_id, academic_year_id, teacher_id, max_students)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    for (const fac of faculty) {
      await db.query(`
        INSERT INTO faculty (id, tenant_id, user_id, employee_id, department_id)
        VALUES (?, ?, ?, ?, ?)
