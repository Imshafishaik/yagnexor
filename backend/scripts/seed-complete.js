...
async function seedDatabase() {
  ...
  try {
    ...
    for (const subject of subjects) {
      await db.query(`
        INSERT INTO subjects (id, tenant_id, name, code, course_id)
        VALUES (?, ?, ?, ?, ?)
    ...
    for (const attendance of attendanceRecords) {
      await db.query(`
        INSERT INTO attendance_records (id, tenant_id, student_id, class_id, date, status, remarks)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    const feeStructures = [
      { id: uuidv4(), name: 'Tuition Fee - CS101', description: 'Semester tuition', amount_due: 1500, fee_type: 'TUITION', academic_year_id: academicYears[1].id, due_date: '2024-01-31', tenant_id: tenantId },
      { id: uuidv4(), name: 'Exam Fee - CS201', description: 'Final exam fee', amount_due: 100, fee_type: 'EXAM', academic_year_id: academicYears[1].id, due_date: '2024-02-28', tenant_id: tenantId },
    ...
    for (const fee of feeStructures) {
      await db.query(`
        INSERT INTO fee_structures (id, tenant_id, name, description, amount_due, fee_type, academic_year_id, due_date)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `, [fee.id, fee.tenant_id, fee.name, fee.description, fee.amount_due, fee.fee_type, fee.academic_year_id, fee.due_date]);
    }
    ...
    for (const studentFee of studentFees) {
      await db.query(`
        INSERT INTO student_fees (id, tenant_id, student_id, fee_structure_id, amount_due, amount_paid, status, due_date, payment_date)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
